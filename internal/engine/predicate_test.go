...
func TestCellPredicateMatchesEveryPopulatedField(t *testing.T) {
	...
	predicate := CellPredicate{
		Text: new("界"), Width: new(2), Fg: new(Color{Kind: ColorPalette, Index: 1}),
		Bg: new(Color{Kind: ColorRGB, R: 0, G: 2, B: 3}), Bold: new(true),
		Dim: new(false), Underline: new(true), Inverse: new(false),
	}
	if !predicate.Matches(cell) {
		t.Fatal("matching predicate failed")
	}
	predicate.Bold = new(false)
	if predicate.Matches(cell) {
...
func TestRegionMatchesClipsWithoutOverflowAndRejectsEmptyIntersection(t *testing.T) {
	...
	predicate := CellPredicate{Text: new("f")}
...
func TestRegionEvaluationSummary(t *testing.T) {
	...
	evaluation := EvaluateRegion(snapshot, &Rect{X: 1, Y: 0, W: 9, H: 1}, RegionMatchAll, CellPredicate{Text: new("a")})
...
func TestRegionMatchesAnyAndAll(t *testing.T) {
	...
	predicate := CellPredicate{Bold: new(true)}
