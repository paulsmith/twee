...
func (s *statusBar) drawLine(line string) {
	...
	_, _ = fmt.Fprintf(s.w, "\x1b7\x1b[%d;1H\x1b[2K%s\x1b8", s.rows, line)
}
func (s *statusBar) line(script *scriptController, trace *traceController) string {
	line := fmt.Sprintf("%s twee wrap │ ^]q quit ^]s script ^]t trace ^]m marker │ script %s │ trace %s", s.mark(script.state), s.scriptText(script), s.traceText(trace))
	if s.toast != "" {
		line += " │ " + s.toast
	}
...
func (s *statusBar) markerPrompt(label string) string {
	return sanitizeStatus("twee wrap │ marker label: "+label+"_ │ Enter save  Esc/Ctrl+C cancel", s.ascii)
}
